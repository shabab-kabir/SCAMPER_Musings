...
; `foo` and `bar`?
;
; Foo and bar have the same output as they are essentially the same function however foo is redundant as it uses boolean values to again output those same boolean values.
...
; "3 2 4" and "2 3 1")?
;
; To see how number positions matter in definitions when booleans are invovled.

; d. What do you see as the similarities and differences between
; `baz` and `qux`?
;
; In essence, both are accomplishing the same task, however baz is redundant due to calling its own boolean value again.
...
(is-even? 12) ; output is #t
(is-even? 11) ; output is #f
...
(define better-both-even?
  (lambda (n1 n2)
    (if (equal? (is-even? n1) (is-even? n2))
      #t
    #f)))
(better-both-even? 2 4)
(better-both-even? 3 2)
(better-both-even? 8 2)
...
;
; <TODO: If <expr> is already a boolean expression it would return a true or false statement, this means no if statement is required. The code would just be (<expr>).
...
(define both-odd?
  (lambda (n1 n2)
    (and (not(is-even? n1)) (not(is-even? n2)))
    ))
...
(define employee-id?
  (lambda (str)
    (cond
        [(> 6 (string-length(str))) #t]
        [(number? str) "Boom! Crash!"]
        [else #f])))
